...
function setup() {
  ...
  outercircle=loadImage("outercircle.png")
  ballimage=loadImage("ball.png");
  world = createWorld();///...changed the gravity inside  box2d-helper js
  world.SetContactListener(new CustomListener());
  tank.push(new gun(width/4,height/2,30,'arihant.jpeg'));
  tank.push(new gun(width*3/4, height/2,30,'photo.jpg'));
  wall.push(new boundary(width/2, height, width, 0));
  wall.push(new boundary(-4, height/2, 0, height));
  wall.push(new boundary(width, height/2, 0, height));
  wall.push(new boundary(width/2, height/2, 10, height/2));
  wall.push(new boundary(width/2, -4 , width , 0));
